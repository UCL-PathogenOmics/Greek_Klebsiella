...
awk '$3=="CDS"{print $1"\t"$4"\t"$5}' "$OUTPUT/${SAMPLE}.gff" > "$OUTPUT/${SAMPLE}_genes.tsv"
...
if samtools view "$OUTPUT/map.bam" | awk '($3!="*"){print $3,$4,$4+length($10)-1; exit}' > "$OUTPUT/mapped_pos.tsv" && [ -s "$OUTPUT/mapped_pos.tsv" ]; then
    read CONTIG START END < "$OUTPUT/mapped_pos.tsv"
else
    echo "No mapping found for $GENE in $SAMPLE" >&2
    exit 1
fi
...
mafft --auto "$OUTPUT/${GENE}_${SAMPLE}_extracted.fasta" > "$OUTPUT/${GENE}_${SAMPLE}_aligned.fasta"
